											...
											<div class="row">
												<div class="col-md-12">
													<div class="form-group">
														...
														<textarea name="note" id="note" rows="2" class="form-control" placeholder="หมายเหตุ">{{ $export->export_note }}</textarea>
													</div>
												</div>
											</div>
										</fieldset>
															...
															<tbody id="rowdata">
																@if(!empty($order))
																	...
																	<tr class="rowproduct" id="row{{ $dataorder->product_id }}">
																		...
																		<td>{{ $dataorder->product_name }}<input type="hidden" name="productid[]" value="{{ $dataorder->product_id }}"><input type="hidden" name="orderid[]" value="{{ $dataorder->order_id }}"></td>
																		...
																		<td align="center">
																			...
																			@foreach($dataorder->unitdata as $item2)
																				...
																				<option value="1,{{$item2->unit_unitfirst}}" @if($dataorder->order_unit == $item2->unit_unitfirst && $dataorder->order_typeunit == 1) selected @endif>{{$item2->unit_name}}</option>
																				<option value="2,{{$item2->unit_unitsec}}" @if($dataorder->order_unit == $item2->unit_unitsec && $dataorder->order_typeunit == 2) selected @endif>{{$item2->unitsub_name}}</option>
																		...
																		<td align="right">
																			{{-- <div>{{ number_format($dataorder->order_price,2) }}</div> --}}
																			<input type="text" class="form-control number" name="productprice[]" id="price{{ $dataorder->product_id }}" value="{{ $dataorder->order_price }}" onkeyup="qtypush({{ $dataorder->product_id }})"></td>
																		
																		<td align="right"><span id="totalprosp{{ $dataorder->product_id }}">{{ number_format($dataorder->order_total,2) }}</span><input type="hidden" name="totalpro[]" id="totalpro{{ $dataorder->product_id }}" value="{{ $dataorder->order_total }}"></td>
																		<td  align="center">
																			<button type="button" class="btn btn-danger btn-rounded" onclick="delrow({{ $dataorder->product_id }})"><i class="icon-cancel-square position-left"></i> Delete</button>
														...
														<div class="col-md-4">
															<div class="form-group">
																...
																<div class="col-md-8">
																	<div class="radio">
																		<label>
																			<input type="radio" class="styled " name="status" id="status1" value="1" > เรียบร้อย
																		</label>
																	</div>
																	<div class="radio">
																		<label>
...
<script>
				...
				$.each(data,function(key,item){
					...
					if(find == 0){
						...
						$('#rowdata').append('<tr class="rowproduct" id="row'+item.id+'"><td align="center">'+item.code+'</td>'
							+'<td>'+item.name+'<input type="hidden" name="productid[]" value="'+item.id+'"><input type="hidden" name="orderid[]" value="{{ $dataorder->order_id }}"></td>'
							...
							+'<td align="right"><div>'+formatNumber(item.price.toFixed(2))+'</div><input type="hidden" class="form-control number" name="productprice[]" id="price'+item.id+'" value="'+item.price+'"></td>'
				...
				'success': function (data) {
					$.each(data,function(key,item){
						...
						if(find == 0){
							...
							select += '</select>'
							$('#rowdata').append('<tr class="rowproduct" id="row'+item.id+'"><td align="center">'+item.code+'</td>'
								+'<td>'+item.name+'<input type="hidden" name="productid[]" value="'+item.id+'"><input type="hidden" name="orderid[]" value=""></td>'
								+'<td align="center"><input type="text" class="form-control number" name="productqty[]" id="qty'+item.id+'" onkeyup="qtypush(\''+item.id+'\')" value="1" style="width:70px"></td>'
								+'<td align="center">'+select+'</td>'
								+'<td align="right"><div>'+formatNumber(item.price.toFixed(2))+'</div><input type="hidden" class="form-control number" name="productprice[]" id="price'+item.id+'" value="'+item.price+'"></td>'
	...
	function delrow(id){
		// var orderid = $('#row'+id+' input[name="orderid[]"]').val();
		// $.get("{{url('export/checkorderinsell')}}/"+orderid, function(data, textStatus, xhr) {
		// 	if(data['sellingdetail_id']){
		// 		Lobibox.notify('error',{
		// 			msg: 'มีข้อมูลนี้ในรายการขายไม่สามารถลบได้',
		// 			buttonsAlign: 'center',
		// 			closeOnEsc: true,  
		// 		});
		// 	}else{
	...
	function changeunit(id){
		...
		$.ajax({
			...
			success:function(data){
				if(data != ''){
					// $("#row"+proid+" td:nth-child(5) div").html(formatNumber(data.toFixed(2)));
					$("#row"+proid+" td:nth-child(5) input[name='productprice[]']").val(data);
	...
	$('input[name="productprice[]"]').bind('copy', function() {
	    console.log('copy behaviour detected!')
	});	
	$('input[name="productprice[]"]').bind('paste', function() {
	    console.log('paste behaviour detected!')
	});	
	$('input[name="productprice[]"]').bind('cut', function() {
	    console.log('cut behaviour detected!')
	});
	$(document).ready(function() {
	 
		$('input[name="productprice[]"]').bind({
			copy : function(){
				console.log('copy behaviour detected!');
			},
			paste : function(){
