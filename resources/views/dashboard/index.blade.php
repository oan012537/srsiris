...
@section('content')
							...
							<div class="panel-body">
								...
								{{-- <div class="col-md-6"> --}}
									<div class="form-group">
										...
										<div class="col-md-2">
											<a href="{{url('dashboard/stock')}}"><button type="button" class="btn btn-primary">ดูข้อสินค้าคงเหลือ</button></a>
										</div>
									</div>
								</div>
							</div>
								...
								<div class="row text-center">
									{{-- <div class="col-md-4">
										<div class="content-group">
											...
											<span class="text-muted text-size-small">ออเดอร์รายสัปดาห์</span>
										</div>
									</div> --}}
									{{-- <div class="col-md-2">
										<h6 class="panel-title" style="margin: auto 20px;">ยอดการขาย</h6>
									</div> --}}
									{{-- <div class="col-md-10"> --}}
										<div class="col-md-4">
											<div class="content-group">
	...
	<script>
		...
		function getdatacharts(){
			datasale = [];
			$.post('dashboard/data',{type:$("#typesearch").val(),sellname:$("#typesearchsell").val(),mount:$("#select_mount").val(),year:$("#select_year").val(),productid:$("#productid").val(),'_token': "{{ csrf_token() }}"}, function(data) {
				// datasales.push(data);
				if($("#typesearch").val() == '1'){
					for (var i = 0; i < 31; i++) {
						datasale.push(i);
				...
				Highchart(data.selling,datasale,data.profit,data.billingnote);
			});
		}
		// console.log(datasales);
		function Highchart(datasales,datasale,profit,billingnote) {
		...
		$("#productname").autocomplete({
			source: "{{url('searchproductnameandemp/autocomplete')}}",
			minLength: 1,
