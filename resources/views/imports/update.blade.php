				...
				<div class="row">
					<div class="col-md-12">
						...
						<div class="panel panel-flat">
							...
							<form id="myForm" method="post" action="{{url('imports/update')}}">
							{{ csrf_field() }}
							<input type="hidden" class="form-control" name="impid" id="impid" value="{{$imports->imp_id}}" readonly>
											...
											<div class="row">
												<div class="col-md-6">
													<div class="form-group">
														...
														<input type="text" class="form-control" name="impno" id="impno" placeholder="เลขที่นำเข้าสินค้า" value="{{$imports->imp_no}}" readonly>
													</div>
												</div>
											...
											<div class="row">
												<div class="col-md-6">
													<div class="form-group">
														...
														<div class="input-group">
															<input type="text" name="docdate" id="docdate" placeholder="วันที่" class="form-control datepicker-dates" onkeydown="return false;" autocomplete="off" value="{{$imports->imp_date}}">
														</div>
													</div>
												</div>
												...
												<div class="col-md-6">
													<div class="form-group">
														...
														<div class="input-group">
															<input type="text" name="empsalename" id="empsalename" class="form-control" onkeydown="return false;" autocomplete="off" value="{{$imports->username}}" readonly>
															<input type="hidden" name="empsaleid" id="empsaleid" class="form-control" onkeydown="return false;" autocomplete="off" value="{{$imports->user_id}}" readonly>
														</div>
													</div>
												</div>
											</div>
										...
										<fieldset>
											...
											<div class="row">
												<div class="col-md-6">
													<div class="form-group">
														...
														<input type="text" class="form-control" name="suppliername" id="suppliername" placeholder="ชื่อผู้ผลิต" autocomplete="off" value="{{$supplier->supplier_name}}">
													</div>
												</div>
												...
												<div class="col-md-6">
													<div class="form-group">
														...
														<input type="text" class="form-control" name="supplier_tax" id="supplier_tax" placeholder="เลขประจำตัวผู้เสียภาษีอากร" autocomplete="off" value="{{$supplier->supplier_tax}}">
													</div>
											...
											<div class="row">
												<div class="col-md-12">
													<div class="form-group">
														<label>ที่อยู่ :</label>
														<textarea name="supplier_address" id="supplier_address" rows="2" class="form-control" placeholder="ที่อยู่">{{$supplier->location}}</textarea>
													</div>
												</div>
											</div>
											...
											<div class="row">
												<div class="col-md-6">
													<div class="form-group">
														...
														<input type="text" class="form-control number" name="supplier_tel" id="supplier_tel" placeholder="เบอร์โทรศัพท์" value="{{$supplier->supplier_tel}}">
													</div>
												</div>
											...
											<div class="row">
												<div class="col-md-12">
													<div class="form-group">
														...
														<textarea name="note" id="note" rows="2" class="form-control" placeholder="หมายเหตุ">{{$imports->impt_note}}</textarea>
													</div>
												</div>
											</div>
										</fieldset>
											...
											<div class="row">
												<div class="col-md-12">
													...
													<table id="myTable" class="table table-framed">
														...
														<tbody id="rowdata">
															@if(!empty($subimports))
																@foreach($subimports as $key => $dataorder)
																<tr class="rowproduct row{{ $dataorder->product_id }}" id="row{{$key}}{{ $dataorder->product_id }}">
																	<td align="center">
																		<input type="hidden" name="importdataid[]" value="{{ $dataorder->sub_id }}">{{ $dataorder->product_code }}
																	</td>
																	<td>{{ $dataorder->product_name }}<input type="hidden" name="proid[]" value="{{ $dataorder->product_id }}"></td>
																	<td align="center">
																		<input type="hidden" class="form-control number" name="oldqty[]" id="oldamount{{ $dataorder->product_id }}"  value="{{ $dataorder->amount }}" required>
																		<input type="text" name="amount[]" class="form-control number" onkeyup="cal()" value="{{ $dataorder->amount }}" required="">
																	</td>
																	<td align="center">
																		<select class="form-control" name="unit[]">
																			<option value="">-- เลือกหน่วยนับ --</option>
																		@if(count($dataorder->unit) > 0)
																		@foreach($dataorder->unit as $units)
																		<option value="1,{{$units->unit_id}}" @if($units->unit_id == $dataorder->importunit) selected @endif>{{$units->unit_name}}</option> {{-- มีแต่หน่วยใหญ่ --}}
																		<option value="2,{{$units->unit_unitsec}}" @if($units->unit_unitsec == $dataorder->importunit) selected @endif>{{$units->unitsub_name}}</option> {{-- หน่วยย่อย --}}
																		@endforeach
																		@else
																		<option value="1,{{$dataorder->unit_id}}" @if($dataorder->unit_id == $dataorder->unit_id) selected @endif>{{$dataorder->unit_name}}</option>
																		@endif
																		</select>
																	</td>
																	<td align="right">
																		<input type="text" name="capital[]" id="capital{{ $dataorder->product_id }}" class="form-control number" onkeyup="capitalpush({{ $dataorder->product_id }});cal();" value="{{ $dataorder->product_capital }}" required>
																	</td>
																	
																	<td  align="center"><button type="button" class="btn btn-danger btn-xs" onclick="delrow('{{$key}}{{ $dataorder->product_id }}')"><i class="icon-cancel-square"></i> </button></td>
																</tr>
																@endforeach
															@endif
														</tbody>
				...
				'success': function(data){
					...
					$.each(data.results,function(key,item){
                        ...
                        $('#rowdata tr').each(function(){
                            if($(this).is('.row'+item.pro_id) == true){
                                find += 1;
                            }                 
                        });
                        // if(find == 0){
                            $('#rowdata').append('<tr class="rowproduct row'+item.pro_id+'" id="row'+find+item.pro_id+'"><td align="center">'+item.pro_code+'</td>'
                                +'<td>'+item.pro_name +item.importdataid+'</td>'
                                // +'<td>'+item.size+'</td>'
